...
rl.on('line', (line) => {
  try {
    ...
    // If parsed has a numeric 'timestamp', normalize it to epoch seconds
    if (parsed && typeof parsed.timestamp === 'number') {
      const ts = parsed.timestamp;
      // Heuristic: scheduler monotonic timestamps are small (< 1e6), epoch is ~1.6e9
      if (ts < 1e6) {
        if (schedulerTimestampOffset === null) {
          // compute offset using current wall-clock epoch time
          schedulerTimestampOffset = (Date.now() / 1000) - ts;
          console.log('Computed schedulerTimestampOffset =', schedulerTimestampOffset);
        }
        // convert monotonic -> epoch
        parsed.timestamp = ts + schedulerTimestampOffset;
      }
      // else it's already epoch-like; leave as-is
    } else {
      // If the scheduler doesn't include timestamp field, add one
